...
func (s *GroupRepoSuite) TestListWithFilters_Platform() {
	...
	groups, _, err := s.repo.ListWithFilters(s.ctx, pagination.PaginationParams{Page: 1, PageSize: 10}, service.PlatformOpenAI, "", "", "")
	s.Require().NoError(err)
...
func (s *GroupRepoSuite) TestListWithFilters_Status() {
	...
	groups, _, err := s.repo.ListWithFilters(s.ctx, pagination.PaginationParams{Page: 1, PageSize: 10}, "", service.StatusDisabled, "", "")
	s.Require().NoError(err)
...
func (s *GroupRepoSuite) TestListWithFilters_IsExclusive() {
	...
	groups, _, err := s.repo.ListWithFilters(s.ctx, pagination.PaginationParams{Page: 1, PageSize: 10}, "", "", "", service.VisibilityPrivate)
	s.Require().NoError(err)
	s.Require().Len(groups, 1)
	s.Require().True(groups[0].IsExclusive)
}

func (s *GroupRepoSuite) TestListWithFilters_LoadsVisiblePlanIDs() {
	group := &service.Group{
		Name:           "visible-plan-list-group",
		Platform:       service.PlatformAnthropic,
		RateMultiplier: 1.0,
	...
	planA, err := s.tx.Client().SubscriptionPlan.Create().
		SetName("visible-plan-list-A").
		SetStatus(service.StatusActive).
		SetVisibility(service.VisibilityPublic).
		Save(s.ctx)
	s.Require().NoError(err)
	planB, err := s.tx.Client().SubscriptionPlan.Create().
		SetName("visible-plan-list-B").
		SetStatus(service.StatusActive).
		SetVisibility(service.VisibilityPublic).
		Save(s.ctx)
	s.Require().NoError(err)

	s.Require().NoError(s.repo.SetVisiblePlans(s.ctx, group.ID, []int64{planB.ID, planA.ID}))
	...
	groups, _, err := s.repo.ListWithFilters(
		...
		"",
		"",
		"visible-plan-list-group",
		"",
	)
	s.Require().NoError(err)
	s.Require().Len(groups, 1)
	s.Require().Equal(group.ID, groups[0].ID)
	s.Require().ElementsMatch([]int64{planA.ID, planB.ID}, groups[0].VisiblePlanIDs)
}
...
func (s *GroupRepoSuite) TestListWithFilters_Search() {
	...
	s.Run("search_name_should_match", func() {
		...
		groups, _, err := repo.ListWithFilters(ctx, pagination.PaginationParams{Page: 1, PageSize: 50}, "", "", "name-target", "")
		s.Require().NoError(err)
	...
	s.Run("search_description_should_match", func() {
		...
		groups, _, err := repo.ListWithFilters(ctx, pagination.PaginationParams{Page: 1, PageSize: 50}, "", "", "desc-needle", "")
		s.Require().NoError(err)
	...
	s.Run("search_nonexistent_should_return_empty", func() {
		...
		groups, _, err := repo.ListWithFilters(ctx, pagination.PaginationParams{Page: 1, PageSize: 50}, "", "", search, "")
		s.Require().NoError(err)
	...
	s.Run("search_should_be_case_insensitive", func() {
		...
		groups, _, err := repo.ListWithFilters(ctx, pagination.PaginationParams{Page: 1, PageSize: 50}, "", "", "mixedcase-needle", "")
		s.Require().NoError(err)
	...
	s.Run("search_should_escape_like_wildcards", func() {
		...
		groups, _, err := repo.ListWithFilters(ctx, pagination.PaginationParams{Page: 1, PageSize: 50}, "", "", "100%", "")
		s.Require().NoError(err)
		...
		groups, _, err = repo.ListWithFilters(ctx, pagination.PaginationParams{Page: 1, PageSize: 50}, "", "", "ab_cd", "")
		s.Require().NoError(err)
...
func (s *GroupRepoSuite) TestListWithFilters_AccountCount() {
	...
	groups, page, err := s.repo.ListWithFilters(s.ctx, pagination.PaginationParams{Page: 1, PageSize: 10}, service.PlatformAnthropic, service.StatusActive, "", service.VisibilityPrivate)
