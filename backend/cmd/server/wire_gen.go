...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	tencentSMSSender := service.NewTencentSMSSender(settingRepository)
	phoneVerificationService := service.NewPhoneVerificationService(smsCache, tencentSMSSender, settingRepository)
	promoCodeRepository := repository.NewPromoCodeRepository(client)
	channelInviteRepository := repository.NewChannelInviteRepository(client)
	...
	oauthDeviceAuthorizationRepository := repository.NewOAuthDeviceAuthorizationRepository(client, db)
	oauthRefreshTokenRepository := repository.NewOAuthRefreshTokenRepository(client, db)
	oauthAccessTokenDenylist := repository.NewOAuthAccessTokenDenylist(redisClient)
	oauthAuthorizationService := service.ProvideOAuthAuthorizationService(oauthClientRepository, oauthAuthorizationCodeRepository, oauthDeviceAuthorizationRepository, oauthRefreshTokenRepository, userRepository, oauthAccessTokenDenylist, configConfig)
	authHandler := handler.NewAuthHandler(configConfig, authService, userService, settingService, promoService, redeemService, totpService, oauthAuthorizationService, channelInviteSvc)
	...
	subscriptionHandler := handler.NewSubscriptionHandler(subscriptionService, subscriptionPlanService)
	...
	adminService := service.NewAdminService(userRepository, groupRepository, subscriptionPlanRepository, accountRepository, soraAccountRepository, proxyRepository, apiKeyRepository, redeemCodeRepository, userGroupRateRepository, billingCacheService, proxyExitInfoProber, proxyLatencyCache, apiKeyAuthCacheInvalidator, client, settingService, subscriptionService, userSubscriptionRepository, privacyClientFactory)
	...
	apiKeyHandler := handler.NewAPIKeyHandler(apiKeyService, pricingService, groupRepository, accountRepository) // MANUAL: model-pricing API with billing model filter
	accountHandler := admin.NewAccountHandler(adminService, oAuthService, openAIOAuthService, geminiOAuthService, antigravityOAuthService, rateLimitService, accountUsageService, accountTestService, concurrencyService, crsSyncService, sessionLimitCache, rpmCache, compositeTokenCacheInvalidator, billingService)
	...
	subscriptionPlanHandler := admin.NewSubscriptionPlanHandler(subscriptionPlanService)
	...
	alipayOrderRepository := repository.NewAlipayOrderRepository(client)
	alipayService := service.NewAlipayService(client, configConfig, alipayOrderRepository, settingRepository, userService)
	adminAlipayHandler := admin.NewAlipayHandler(alipayService)
	channelInviteHandler := admin.NewChannelInviteHandler(channelInviteSvc)
	userOperationsService := service.NewUserOperationsService(client, userRepository, redeemCodeRepository, usageLogRepository)
	userOperationsHandler := admin.NewUserOperationsHandler(userOperationsService)
	adminHandlers := handler.ProvideAdminHandlers(dashboardHandler, adminUserHandler, groupHandler, accountHandler, adminAnnouncementHandler, dataManagementHandler, backupHandler, oAuthHandler, openAIOAuthHandler, geminiOAuthHandler, antigravityOAuthHandler, proxyHandler, adminRedeemHandler, promoHandler, settingHandler, opsHandler, systemHandler, adminSubscriptionHandler, adminUsageHandler, userAttributeHandler, errorPassthroughHandler, tlsFingerprintProfileHandler, adminAPIKeyHandler, scheduledTestHandler, subscriptionPlanHandler, adminAlipayHandler, channelInviteHandler, userOperationsHandler)
	usageRecordWorkerPool := service.NewUsageRecordWorkerPool(configConfig)
	userMsgQueueCache := repository.NewUserMsgQueueCache(redisClient)
	userMessageQueueService := service.ProvideUserMessageQueueService(userMsgQueueCache, rpmCache, configConfig)
	oauthProfileService := service.NewOAuthProfileService(apiKeyRepository, userRepository)
	gatewayHandler := handler.NewGatewayHandler(gatewayService, geminiMessagesCompatService, antigravityGatewayService, userService, concurrencyService, billingCacheService, usageService, apiKeyService, oauthProfileService, usageRecordWorkerPool, errorPassthroughService, userMessageQueueService, configConfig, settingService)
	...
	ciHandler := handler.NewChannelInviteHandler(channelInviteSvc)
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, adminHandlers, gatewayHandler, openAIGatewayHandler, soraGatewayHandler, soraClientHandler, handlerSettingHandler, totpHandler, alipayHandler, pricingHandler, inviteHandler, ciHandler, idempotencyCoordinator, idempotencyCleanupService)
	jwtAuthMiddleware := middleware.NewJWTAuthMiddleware(authService, userService)
	adminAuthMiddleware := middleware.NewAdminAuthMiddleware(authService, userService, settingService)
	apiKeyAuthMiddleware := middleware.NewAPIKeyAuthMiddlewareWithOAuth(apiKeyService, subscriptionService, oauthAuthorizationService, configConfig)
	...
	subscriptionExpiryService := service.ProvideSubscriptionExpiryService(userSubscriptionRepository, billingCache, subscriptionService)
